...
def analyse() :
    for cmd in ("samtools view -S -b mapping.sam > mapping.bam", #SAM --> BAM (= + d'espace de stockage et + processing plus rapide)
                ...
                "samtools view -f 0x1 -f 0x4 -F 0x8 mapping.sorted.bam | echo 'Nombre de paires de read avec le read unmapped et le mate mapped:' $(wc -l) >> summary_file.txt",
                "samtools view -f 0x1 -f 0x8 -F 0x4 mapping.sorted.bam | echo 'Nombre de paires de read avec le read mapped et le mate unmapped:' $(wc -l) >> summary_file.txt",
                
                ################################ paires de reads où un seul read de la paire est entièrement mappé et l'autre partiellement mappé
                
                "samtools view -f 0x1 -f 0x2 -F 0x8 mapping.sorted.bam | grep -v '100M' > read_mapped__mate_partially_mapped.fasta",
                "samtools view -f 0x1 -F 0x2 -F 0x8 mapping.sorted.bam | grep -v '100M' > read_partially_mapped__mate_mapped.fasta",

                "samtools view -f 0x1 -f 0x2 -F 0x8 mapping.sorted.bam | grep -v '100M' | echo 'Nombre de paires de read avec le read mapped et le mate partially mapped:' $(wc -l) >> summary_file.txt",
                "samtools view -f 0x1 -F 0x2 -F 0x8 mapping.sorted.bam | grep -v '100M' | echo 'Nombre de paires de read avec le read partially mapped et le mate mapped:' $(wc -l) >> summary_file.txt",
